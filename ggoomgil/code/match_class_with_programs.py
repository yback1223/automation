...
src_data_classifications = "../srcs/classifications.json"
src_data_programs = "programs_with_classifications.json"
# src_data_programs = "../srcs/programs.json"
output_data = "programs_with_classifications.json"
...
class MultiDepthCrawler:
    ...
    def choose_classification(self, classification):
        try:
            ...
            print(f"Error selecting classification")





class ClassProgramMatcher:

    def __init__(self):
        # 기존 JSON 불러오기
        print(f'programs_with_classifications.json 로드 중...')
        self.programs = self.load_existing_data(src_data_programs)

        print(f'classifications.json 로드 중...')
        self.classifications = self.load_existing_data(src_data_classifications)
    ...
    def run(self):
        try:
            ...
            for one_classification in self.classifications:
                ...
                while True:
                    ...
                    for program_row in program_rows:
                        try:
                            ...
                            existing_programs = [program for program in self.programs if program.get("program_name") == program_name]                           
                            is_duplicate = False
                            for existing_program in existing_programs:
                                existing_program_classes = existing_program.get("classes", [])

                                for existing_class in existing_program_classes:
                                    if (
                                        existing_class.get("depth_1") == one_classification.get("depth_1") and
                                        existing_class.get("depth_2") == one_classification.get("depth_2") and
                                        existing_class.get("depth_3") == one_classification.get("depth_3") and
                                        existing_class.get("depth_4") == one_classification.get("depth_4")
                                    ):
                                        is_duplicate = True
                                        break

                                if not is_duplicate:
                                    print(f"Inserting classification for program: {program_name}")

                                    if not existing_program_classes:
                                        existing_program["classes"] = [{
                                        ...
                                        existing_program["classes"].append({
                                            "depth_1": one_classification.get("depth_1", ""),
                                            "depth_2": one_classification.get("depth_2", ""),
                                            "depth_3": one_classification.get("depth_3", ""),
                                            "depth_4": one_classification.get("depth_4", "")
                            ...
                            if is_duplicate:
                                print(f"Skipping duplicate classification for program: {program_name}")
                                continue
                ...
                time.sleep(WORK_TERM_SLEEP)
                self.save_to_json(output_data, self.programs)
    ...
    def save_to_json(self, file_name, data):
        try:
            ...
            print(f"Error saving data to JSON")
    ...
    def navigate_to_next_page(self):
        try:
            ...
            print(f"Error navigating to next page")
            return False
